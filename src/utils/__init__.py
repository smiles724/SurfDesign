...
from src.utils import strategies
from src.utils.config import load_yaml_config, instantiate_from_config, resolve_experiment_config
...
@rank_zero_only
def print_config(config: DictConfig, print_order: Sequence[str] = ("datamodule", "task", "model", "callbacks", "logger", "trainer", "training"), resolve: bool = True, ) -> None:
...
@rank_zero_only
def log_hyperparameters(config: DictConfig, model: pl.LightningModule, datamodule: pl.LightningDataModule, trainer: pl.Trainer, callbacks: List[pl.Callback],
        logger: List[pl.loggers.Logger], ) -> None:
...
def finish(config: DictConfig, model: pl.LightningModule, datamodule: pl.LightningDataModule, trainer: pl.Trainer, callbacks: List[pl.Callback],
        logger: List[pl.loggers.Logger], ) -> None:
...
def common_pipeline(config):
    ...
    if "callbacks" in config:
        ...
        if config.trainer.get('enable_progress_bar', False):
            ...
            callbacks.append(RichProgressBar(leave=False))
