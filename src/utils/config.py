...
def merge_config(default_cfg, override_cfg):    # combine the key of inputs in two cfgs
...
def instantiate_from_config(cfg: OmegaConf, group=None, **override_kwargs):
    ...
    if group is None:
        ...
        if target is None:
            raise KeyError(f'{_target_} is not a registered <{group}> class [{registry.get_registered_modules(group)}].')
