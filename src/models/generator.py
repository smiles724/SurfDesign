...
class IterativeRefinementGenerator(object):
    def __init__(self, alphabet=None, max_iter=1, strategy='denoise', temperature=None, **kwargs):
    ...
    @torch.no_grad()
    def generate(self, model, batch, max_iter=None, strategy=None, temperature=None, replace_visible_tokens=False, need_attn_weights=False):
        ...
        initial_output_tokens, initial_output_scores = model.initialize_output_tokens(batch, encoder_out=encoder_out)
        prev_decoder_out = dict(output_tokens=initial_output_tokens, output_scores=initial_output_scores, output_masks=None, attentions=None, step=0, max_step=max_iter,
                                history=[initial_output_tokens.clone()], temperature=temperature, )
        ...
        for step in range(max_iter):
            ...
            # 2.1: predict
            decoder_out = model.forward_decoder(prev_decoder_out=prev_decoder_out, encoder_out=encoder_out, need_attn_weights=need_attn_weights)
            output_tokens = decoder_out['output_tokens']
            output_scores = decoder_out['output_scores']

            # 2.2: re-mask skeptical parts of low confidence skeptical decoding (depend on the maximum decoding steps.)
            if strategy == 'mask_predict' and (step + 1) < max_iter:
                skeptical_mask = _skeptical_unmasking(output_scores=output_scores, output_masks=output_tokens.ne(self.padding_idx),  # & coord_mask,
                                                      p=1 - (step + 1) / max_iter)
