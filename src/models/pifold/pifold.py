...
class PiFold(FixedBackboneDesignEncoderDecoder):
    ...
    def forward(self, batch, return_feats=False, **kwargs):
        logits, feats = self.model(X=batch['coords'], mask=batch['coord_mask'].float(), S=batch['prev_tokens'], lengths=batch.get('lengths', None))
    ...
    def forward_encoder(self, batch):
        encoder_out = self.model.encode(X=batch['coords'], mask=batch['coord_mask'].float(), lengths=batch['lengths'])
    ...
    def forward_decoder(self, prev_decoder_out, encoder_out):
        ...
        logits, _ = self.model.decode(prev_tokens=output_tokens, encoder_out=encoder_out, )
