...
from .modules import *
from .modules import _dihedrals, _get_rbf, _orientations_coarse_gl_tuple
...
class PiFoldModel(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        self.W_v = nn.Sequential(nn.Linear(node_features, hidden_dim, bias=True), nn.LeakyReLU(), nn.BatchNorm1d(hidden_dim), nn.Linear(hidden_dim, hidden_dim, bias=True),
            nn.LeakyReLU(), nn.BatchNorm1d(hidden_dim), nn.Linear(hidden_dim, hidden_dim, bias=True))
    ...
    def encode(self, X, mask, lengths, S=None):
        X, S, score, h_V, h_P, P_idx, batch_id, mask_bw, mask_fw, decoding_order = self._get_features(S=S, X=X, score=None, mask=mask)
        h_V = self.W_v(self.norm_nodes(self.node_embedding(h_V)))
        h_P = self.W_e(self.norm_edges(self.edge_embedding(h_P)))

        return {'node_feats': h_V, 'edge_feats': h_P, 'edge_idx': P_idx, 'batch_id': batch_id, 'node_mask': mask}
...
def length_to_mask(lens, maxlen=None):
    # lens: (bsz)
