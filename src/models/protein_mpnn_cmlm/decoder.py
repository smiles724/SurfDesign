...
class MPNNDecoder(nn.Module):
    def __init__(self, hidden_dim, num_decoder_layers=3, vocab=22, k_neighbors=64, dropout=0.1, random_decoding_order=True, nar=False, token_embed=None, out_proj=None):
        super().__init__()
    ...
    def _prepare_decoding_order(self, mask, E_idx, chain_mask=None, decoding_order=None):
        ...
        if not self.nar:
            ...
            order_mask_backward = torch.einsum('ij, biq, bjp->bqp', 1 - torch.triu(torch.ones(mask_size, mask_size, device=device)),  # low triangular without diagonal, the AR mask
    ...
    def forward(self, h_S, h_V, h_E, E_idx, mask, chain_mask=None, decoding_order=None):
        ...
        mask_fw, mask_bw, deocding_order, chain_mask = self._prepare_decoding_order(mask, E_idx, decoding_order=decoding_order, chain_mask=chain_mask, )
    ...
    def sample(self, h_V, h_E, E_idx, mask, h_S=None, chain_mask=None, decoding_order=None, temperature=1e-2):
        if self.nar:
            return self.sample_nar(h_S, h_V, h_E, E_idx, mask, chain_mask=chain_mask, decoding_order=decoding_order, temperature=temperature)
        device = mask.device
        N_batch, N_nodes = mask.shape[0], mask.shape[1]

        # get causal masks if random decoding order
        mask_fw, mask_bw, decoding_order, chain_mask = self._prepare_decoding_order(mask, E_idx, decoding_order=decoding_order, chain_mask=chain_mask)
        ...
        for t_ in range(N_nodes):
            ...
            for l, layer in enumerate(self.decoder_layers):
                ...
                h_V_stack[l + 1].scatter_(1, t[:, None, None].repeat(1, 1, h_V.shape[-1]), layer(h_V_t, h_ESV_t, mask_V=mask_t)  # layer update
                                          )
...
class SequenceDecoder(nn.Module):
    ...
    def forward(self, prev_tokens: torch.FloatTensor, prev_token_padding_mask: torch.BoolTensor, memory: torch.FloatTensor = None, memory_padding_mask: torch.BoolTensor = None,
                **kwargs):
...
class MPNNSequenceDecoder(SequenceDecoder):
    def __init__(self, n_vocab, d_model, n_layers, dropout=0.1, random_decoding_order=True, nar=False, crf=False, alphabet=None, token_embed=None, out_proj=None) -> None:
        super().__init__(n_vocab, d_model)
        ...
        self.nar = nar
        self.mpnn_decoder = MPNNDecoder(hidden_dim=d_model, num_decoder_layers=n_layers, random_decoding_order=random_decoding_order, dropout=dropout, token_embed=self.token_embed,
                                        out_proj=self.out_proj, nar=nar)

        if crf:
            from torch_random_fields.models import GeneralCRF
            from torch_random_fields.models.constants import Training, Inference
            self.crf = GeneralCRF(num_states=n_vocab, feature_size=d_model, beam_size=64, low_rank=n_vocab, training=Training.PIECEWISE,
    ...
    def run_crf(self, logits, masks, edge_idx, node_features=None, max_binary_edges=None, target_tokens=None, decoding=False):
        ...
        if not decoding:
            crf_loss = self.crf(unaries=logits, masks=masks, binary_edges=binary_edges, binary_masks=binary_masks.to(logits), targets=target_tokens, node_features=node_features, )
            return crf_loss
        else:
            output_tokens = self.crf(unaries=logits, masks=masks, binary_edges=binary_edges, binary_masks=binary_masks.to(logits), node_features=node_features, )[1]
            return output_tokens

    def forward(self, prev_tokens: torch.LongTensor, prev_token_padding_mask: torch.BoolTensor = None, memory: torch.FloatTensor = None, memory_mask: torch.FloatTensor = None,
                memory_padding_mask: torch.BoolTensor = None, target_tokens: torch.LongTensor = None, chain_mask=None, **kwargs):
        ...
        feats = self.mpnn_decoder(h_S=h_tokens, h_V=node_feats, h_E=edge_feats, E_idx=edge_idx, mask=memory_mask, chain_mask=chain_mask)
        logits = self.out_proj(feats)

        if exists(self.crf):
            crf_loss = self.run_crf(logits=logits, masks=memory_mask, edge_idx=edge_idx, node_features=feats, target_tokens=target_tokens, )
            return logits, crf_loss
        return logits, {'feats': feats, **memory}

    def sample(self, prev_tokens: torch.LongTensor = None, prev_token_padding_mask: torch.BoolTensor = None, memory: torch.FloatTensor = None,
               memory_mask: torch.FloatTensor = None, memory_padding_mask: torch.BoolTensor = None, **kwargs):
        if self.nar:
    ...
    def sample_nar(self, prev_tokens: torch.LongTensor = None, prev_token_padding_mask: torch.BoolTensor = None, memory: torch.FloatTensor = None,
                   memory_mask: torch.FloatTensor = None, memory_padding_mask: torch.BoolTensor = None, **kwargs):
        ...
        for step in range(max_step):
            ...
            decoder_out = self.mpnn_decoder(h_S=h_tokens, h_V=node_feats.clone(), h_E=edge_feats.clone(), E_idx=edge_idx.clone(), mask=memory_mask.clone())

            if exists(self.crf):
                _tokens = self.run_crf(logits=self.out_proj(decoder_out), masks=memory_mask.bool(), edge_idx=edge_idx, node_features=decoder_out, decoding=True)
                _scores = output_scores.clone()
            else:
                log_probs = torch.log_softmax(self.out_proj(decoder_out), dim=-1)
                _scores, _tokens = log_probs.max(-1)

            output_tokens.masked_scatter_(output_masks, _tokens[output_masks])
            output_scores.masked_scatter_(output_masks, _scores[output_masks])
            if (step + 1) < max_step:
                skeptical_mask = _skeptical_unmasking(output_scores=output_scores, output_masks=output_tokens.ne(self.pad),  # & coord_mask,
                                                      p=1 - (step + 1) / max_step)
    ...
    def initialize_output_tokens(self, prev_tokens, mask=None):
        ...
        if mask is not None:
            initial_output_tokens = torch.where(~mask, prev_tokens, initial_output_tokens)
    ...
    def sample_ar(self, prev_tokens: torch.LongTensor = None, prev_token_padding_mask: torch.BoolTensor = None, memory: torch.FloatTensor = None,
                  memory_mask: torch.FloatTensor = None, memory_padding_mask: torch.BoolTensor = None, **kwargs):
        node_feats, edge_feats, edge_idx = memory['node_feats'], memory['edge_feats'], memory['edge_idx']
        ...
        preds = self.mpnn_decoder.sample(h_V=node_feats, h_E=edge_feats, E_idx=edge_idx, mask=memory_mask)
        return preds
