...
from src.models.generator import new_arange, sample_from_categorical
...
class ProteinMPNNCMLM(FixedBackboneDesignEncoderDecoder):
    ...
    def __init__(self, cfg) -> None:
        super().__init__(cfg)
        self.encoder = MPNNEncoder(node_features=self.cfg.d_node_feats, edge_features=self.cfg.d_edge_feats, hidden_dim=self.cfg.d_model, num_encoder_layers=self.cfg.n_enc_layers,
                                   k_neighbors=self.cfg.k_neighbors, augment_eps=self.cfg.augment_eps, dropout=self.cfg.dropout)
        ...
        self.decoder = MPNNSequenceDecoder(n_vocab=self.cfg.n_vocab, d_model=self.cfg.d_model, n_layers=self.cfg.n_dec_layers, random_decoding_order=self.cfg.random_decoding_order,
                                           dropout=self.cfg.dropout, nar=self.cfg.nar, crf=self.cfg.crf, alphabet=alphabet)

    def forward(self, batch, init_feat=None, return_feats=False, **kwargs):
        coord_mask = batch['coord_mask'].float()
        encoder_out = self.encoder(X=batch['coords'], mask=coord_mask, init_feat=init_feat, residue_idx=batch.get('residue_idx', None), chain_idx=batch.get('chain_idx', None))
        logits, feats = self.decoder(prev_tokens=batch['prev_tokens'], memory=encoder_out, memory_mask=coord_mask, target_tokens=batch.get('tokens'), **kwargs)
        if return_feats:
            return logits, feats
        return logits

    def forward_encoder(self, batch):
        encoder_out = self.encoder(X=batch['coords'], mask=batch['coord_mask'].float(), residue_idx=batch.get('residue_idx', None), chain_idx=batch.get('chain_idx', None))
    ...
    def forward_decoder(self, prev_decoder_out, encoder_out, need_attn_weights=False):
        ...
        logits, _ = self.decoder(prev_tokens=output_tokens, memory=encoder_out, memory_mask=encoder_out['coord_mask'].float(), )
        ...
        return dict(output_tokens=output_tokens, output_scores=output_scores, step=step + 1, max_step=max_step, history=history)
