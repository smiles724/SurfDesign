...
from .lora import decorate_lora
from src.utils.config import compose_config as Cfg, merge_config


class ESM2WithStructuralAdatper(nn.Module):
    @classmethod
    def from_pretrained(cls, args, name='esm2_t33_650M_UR50D', target_modules=["q_proj", "k_proj", "v_proj", "out_proj", "fc1", "fc2", "dense"]):
        ...
        if args.use_lora:
            decorate_lora(model, target_modules, args.lora_r, args.lora_alpha)
        return model
    ...
    def _init_layer(self, layer_idx):
        if layer_idx in self.args.adapter_layer_indices:
            return TransformerLayerWithStructuralAdapter(self.embed_dim, 4 * self.embed_dim, self.attention_heads, add_bias_kv=False, use_esm1b_layer_norm=True,
                                                         use_rotary_embeddings=True, encoder_embed_dim=self.args.encoder.d_model, dropout=self.args.dropout)
    ...
    def forward(self, tokens, encoder_out, repr_layers=[], ):
        ...
        x = self.lm_head(x)
        return {"logits": x, "representations": hidden_rep}
...
class TransformerLayerWithStructuralAdapter(nn.Module):
    ...
    def forward(self, x, encoder_out, self_attn_mask=None, self_attn_padding_mask=None, need_head_weights=False):
        ...
        x += self.forward_adapter(x, encoder_out, attn_mask=self_attn_mask, attn_padding_mask=self_attn_padding_mask)  # additional adapter
        x += residual
        return x, attn

    def forward_adapter(self, x, encoder_out, attn_mask, attn_padding_mask):
        encoder_feats = encoder_out['feats'].transpose(0, 1)  # (B, L, D) -> (L, B, D)
