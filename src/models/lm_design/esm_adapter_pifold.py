...
class ESMAdapterPiFold(FixedBackboneDesignEncoderDecoder):
    ...
    def forward_encoder(self, batch):
        encoder_logits, encoder_out = self.encoder(batch['coords'], batch['coord_mask'], batch['prev_tokens'], return_feats=True)
    ...
    def forward_decoder(self, prev_decoder_out, encoder_out, need_attn_weights=False):
        ...
        esm_logits = self.decoder(# tokens=encoder_out['init_pred'],
            tokens=output_tokens, encoder_out=encoder_out, )['logits']
