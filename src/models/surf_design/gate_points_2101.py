...
from .utils import smooth_cutoff, scatter
...
def radius_graph(pos, r_max, batch) -> torch.Tensor:
    ...
    # large batch sizes and the INT_MAX error https://kevinmusgrave.github.io/pytorch-metric-learning/faq/
...
class Convolution(torch.nn.Module):
    ...
    def __init__(self, irreps_in, irreps_node_attr, irreps_edge_attr, irreps_out, number_of_basis, radial_layers, radial_neurons, num_neighbors, ) -> None:
        super().__init__()
...
class Network(torch.nn.Module):
    ...
    def __init__(self, irreps_in: Optional[o3.Irreps], irreps_hidden: o3.Irreps, irreps_out: o3.Irreps, irreps_node_attr: o3.Irreps, irreps_edge_attr: Optional[o3.Irreps],
                 layers: int, max_radius: float, number_of_basis: int, radial_layers: int, radial_neurons: int, num_neighbors: float, num_nodes: float,
        ...
        for _ in range(layers):
            ...
            conv = Convolution(irreps, self.irreps_node_attr, self.irreps_edge_attr, gate.irreps_in, number_of_basis, radial_layers, radial_neurons, num_neighbors, )
            irreps = gate.irreps_out
            self.layers.append(Compose(conv, gate))

        self.layers.append(Convolution(irreps, self.irreps_node_attr, self.irreps_edge_attr, self.irreps_out, number_of_basis, radial_layers, radial_neurons, num_neighbors, ))
