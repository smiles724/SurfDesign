...
class Convolution(torch.nn.Module):
    ...
    def __init__(self, irreps_in: o3.Irreps, irreps_node_attr: o3.Irreps, irreps_edge_attr: o3.Irreps, irreps_out: Optional[o3.Irreps], number_of_edge_features: int,
                 radial_layers: int, radial_neurons: int, num_neighbors: float, ) -> None:
        super().__init__()
        ...
        tp = TensorProduct(self.irreps_in, self.irreps_edge_attr, irreps_mid, instructions, internal_weights=False, shared_weights=False, )
        self.fc = FullyConnectedNet([number_of_edge_features] + radial_layers * [radial_neurons] + [tp.weight_numel], torch.nn.functional.silu)
        self.tp = tp
...
class Network(torch.nn.Module):
    ...
    def __init__(self, irreps_in: o3.Irreps, irreps_hidden: o3.Irreps, irreps_out: o3.Irreps, irreps_node_attr: o3.Irreps, irreps_edge_attr: o3.Irreps, layers: int,
                 max_radius: float, number_of_basis: int, radial_layers: int, radial_neurons: int, num_neighbors: float, num_nodes: float, reduce_output: bool = True, ) -> None:
        super().__init__()
        ...
        for _ in range(layers):
            ...
            gate = Gate(irreps_scalars, [act[ir.p] for _, ir in irreps_scalars],  # scalar
                        irreps_gates, [act_gates[ir.p] for _, ir in irreps_gates],  # gates (scalars)
                        irreps_gated,  # gated tensors
                        )
            conv = Convolution(irreps, self.irreps_node_attr, self.irreps_edge_attr, gate.irreps_in, number_of_edge_features, radial_layers, radial_neurons, num_neighbors, )
            irreps = gate.irreps_out
            self.layers.append(Compose(conv, gate))

        self.layers.append(
            Convolution(irreps, self.irreps_node_attr, self.irreps_edge_attr, self.irreps_out, number_of_edge_features, radial_layers, radial_neurons, num_neighbors, ))
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        edge_length_embedded = soft_one_hot_linspace(x=edge_length, start=0.0, end=self.max_radius, number=self.number_of_basis, basis="gaussian", cutoff=False).mul(
