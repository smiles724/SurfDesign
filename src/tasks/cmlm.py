...
from src.tasks._base import TaskLitModule, register_task
...
class CMLM(TaskLitModule):
    _DEFAULT_CFG: DictConfig = Cfg(learning=Cfg(noise='no_noise', num_unroll=0, ), generator=Cfg(max_iter=1, strategy='denoise',  # ['denoise' | 'mask_predict']
                                                                                                 noise='full_mask',  # ['full_mask' | 'selected mask']
                                                                                                 replace_visible_tokens=False, temperature=0, eval_sc=False, ))

    def __init__(self, model: Union[nn.Module, DictConfig], alphabet: DictConfig, criterion: Union[nn.Module, DictConfig], optimizer: DictConfig, lr_scheduler: DictConfig = None,
                 *, learning=_DEFAULT_CFG.learning, generator=_DEFAULT_CFG.generator):  # nq
        ...
        self.hparams.generator = merge_config(default_cfg=self._DEFAULT_CFG.generator, override_cfg=self.hparams.generator)
        self.generator = IterativeRefinementGenerator(alphabet=self.alphabet, **self.hparams.generator)

    def setup(self, stage=None, verbose=False) -> None:
        """ Called at the beginning of fit and test. """
        super().setup(stage)
        self.build_criterion()
        self.build_torchmetric()
        if self.stage == 'fit' and verbose:
            log.info(f'\n{self.model}')
    ...
    def inject_noise(self, tokens, coord_mask, noise=None, sel_mask=None, mask_by_unk=False):
        ...
        def _full_mask(target_tokens):
            target_mask = (target_tokens.ne(padding_idx) & target_tokens.ne(self.alphabet.cls_idx) & target_tokens.ne(self.alphabet.eos_idx))
    ...
    def step(self, batch):
        ...
        prev_tokens, prev_token_mask = self.inject_noise(tokens, coord_mask, noise=self.hparams.learning.noise)
        ...
        if isinstance(logits, tuple):
            ...
            loss, logging_output = self.criterion(logits, tokens,  # hack to calculate ppl over coord_mask in test as same other methods
            ...
            loss += encoder_loss   # auxiliary loss for the encoder, TODO: delete this loss or give a small weight
    ...
    def on_validation_epoch_end(self, outputs: List[Any]):
        ...
        if self.stage == 'fit':
            ...
            self.on_predict_epoch_end(results=None)
    ...
    def forward(self, batch, return_ids=False):
        ...
        # NOTE: 'full_mask' by default. Set to 'selected_mask' when doing inpainting.
        prev_tokens, prev_token_mask = self.inject_noise(tokens, batch['coord_mask'], noise=self.hparams.generator.noise)
        batch['prev_tokens'] = prev_tokens
        batch['prev_token_mask'] = prev_tokens.eq(self.alphabet.mask_idx)

        output_tokens, output_scores = self.generator.generate(model=self.model, batch=batch, max_iter=self.hparams.generator.max_iter, strategy=self.hparams.generator.strategy,
                                                               replace_visible_tokens=self.hparams.generator.replace_visible_tokens, temperature=self.hparams.generator.temperature)
        if not return_ids:
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0, log_metrics=True) -> Any:
        ...
        if log_metrics:
            ...
            self.acc_median.update(recovery_acc_per_sample)   # TODO: rename, not 'median' but 'global'

            # global accuracy
            recovery_acc = metrics.accuracy(pred_tokens, tokens, mask=coord_mask)
            self.acc.update(recovery_acc, weight=coord_mask.sum())

        results = {'pred_tokens': pred_tokens, 'names': batch['names'], 'native': batch['seqs'], 'recovery': recovery_acc_per_sample, 'sc_tmscores': np.zeros(pred_tokens.shape[0])}
        if self.hparams.generator.eval_sc:
    ...
    def save_prediction(self, results, saveto=None):
        ...
        for entry in results:
            for name, prediction, native, recovery, scTM in zip(entry['names'], self.alphabet.decode(entry['pred_tokens'], remove_special=True), entry['native'], entry['recovery'],
                                                                entry['sc_tmscores'], ):
                save_dict[name] = {'prediction': prediction, 'native': native, 'recovery': recovery}
                if saveto:
    ...
    def eval_self_consistency(self, pred_ids, positions, mask=None):
        ...
        with torch.no_grad():
            ...
            for i in range(positions.shape[0]):
                ...
                seqlen = len(pred_seq)
                _, sc_tmscore = metrics.calc_tm_score(positions[i, 1:seqlen + 1, :3, :].cpu().numpy(), output['positions'][-1, i, :seqlen, :3, :].cpu().numpy(), pred_seq, pred_seq)
