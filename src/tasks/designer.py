...
from src.datamodules.datasets import DataProcessor
...
def _full_mask(target_tokens, coord_mask, alphabet):
    target_mask = (target_tokens.ne(alphabet.padding_idx)  # & mask
                   & target_tokens.ne(alphabet.cls_idx) & target_tokens.ne(alphabet.eos_idx))
...
class Designer:
    _default_cfg = Cfg(cuda=False, generator=Cfg(max_iter=1, strategy='denoise',  # ['denoise' | 'mask_predict']
                                                 temperature=0, eval_sc=False, ))

    def __init__(self, experiment_path, cfg: DictConfig = None):
    ...
    def _initialize(self):
        pl_task, exp_cfg = utils.load_from_experiment(self.experiment_path)
        ...
        self.cfg.generator = utils.config.merge_config(pl_task.hparams.generator, self.cfg.generator)
        self.generator = IterativeRefinementGenerator(alphabet=self.alphabet, **self.cfg.generator)
    ...
    def set_structure(self, pdb_path, chain_list=[], masked_chain_list=None, verbose=False):
        pdb_id = Path(pdb_path).stem

        print(f'loading backbone structure from {pdb_path}.')
        # coords, native_seq = io.load_coords(pdb_path, None)
        self._structure = self.data_processor.parse_PDB(pdb_path, input_chain_list=chain_list, masked_chain_list=masked_chain_list)
        if verbose:
    ...
    def _featurize(self):
        ...
        prev_tokens, prev_token_mask = _full_mask(batch['tokens'], batch['coord_mask'], self.alphabet)
    ...
    def generate(self, generator_args={}, need_attn_weights=False):
        ...
        outputs = self.generator.generate(model=self.model, batch=batch, need_attn_weights=need_attn_weights, **generator_args)

        output_tokens = outputs[0]
        output_tokens = self.alphabet.decode(output_tokens, remove_special=True)

        self._predictions = GenOut(output_tokens=output_tokens, output_scores=outputs[1], attentions=outputs[2] if need_attn_weights else None)
    ...
    def inpaint(self, start_ids, end_ids, generator_args={}, need_attn_weights=False):
        ...
        outputs = self.generator.generate(model=self.model, batch=batch, need_attn_weights=need_attn_weights, replace_visible_tokens=True, **generator_args)
        output_tokens = outputs[0]

        original_segments = []
        designed_segments = []
        for sid, eid in zip(start_ids, end_ids):
            original_segment = self.alphabet.decode(batch['tokens'][..., sid:eid + 1].clone(), remove_special=False)
            original_segments.append(original_segment)

            designed_segment = self.alphabet.decode(output_tokens[..., sid:eid + 1].clone(), remove_special=False)
